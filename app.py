...
if archivo_subido is not None and api_key:
    # Leer datos y limpiar nombres de columnas (quita espacios invisibles)
    df = pd.read_excel(archivo_subido)
    df.columns = df.columns.str.strip() 
    
    # Asegurarnos de usar solo filas con coordenadas válidas
    if 'Coordenadas' in df.columns:
        ...
        if 'Día' in df.columns:
            ...
            if 'Ruta' in df_dia.columns:
                ...
                if st.button("🗺️ Calcular y Generar Mapa"):
                    if not rutas_seleccionadas:
                        ...
                        with st.spinner("Calculando las rutas óptimas por las calles..."):
                            ...
                            # Crear el mapa base centrado en el primer punto
                            ...
                            # Colores para diferenciar capas
                                    ...
                                    if solution:
                                        ...
                                        if response_rutas.status_code == 200:
                                            ...
                                            for paso, nodo in enumerate(nodos_ordenados):
                                                ...
                                                # Uso de .get() para evitar KeyErrors si la columna no existe o tiene otro nombre
                                                depto = fila.get('Departamento', 'Sin dato')
                                                lugar = fila.get('Lugar', 'Sin dato')
                                                dia = fila.get('Día', 'Sin dato')
                                                ruta_nombre = fila.get('Ruta', 'Sin dato')
                                                
                                                popup_html = f"<b>Orden:</b> {paso+1}<br><b>Día:</b> {dia}<br><b>Ruta:</b> {ruta_nombre}<br><b>Depto:</b> {depto}<br><b>Lugar:</b> {lugar}"
                ...
                st.error("No se encontró la columna 'Ruta' en el archivo.")
        else:
            st.error("No se encontró la columna 'Día' en el archivo.")
    else:
        st.error("No se encontró la columna 'Coordenadas' en el archivo.")

elif not api_key:
    st.info("👈 Por favor, ingresa tu API Key en la barra lateral.")
